...
public class Quiz2 {
    ...
    public Map<String, Integer> quiz1() {
        return WORDS.stream().map(word -> word.substring(0,1))
                .collect(Collectors.toMap(word -> word, word -> 1, (ordValue,newValue) -> ordValue += newValue));
    }

    // 2.2 List에 저장된 단어들 중에서 단어의 길이가 2 이상인 경우에만, 모든 단어를 대문자로 변환하여 스페이스로 구분한 하나의 문자열로 합한 결과를 반환하여라.
    // ex) ["Hello", "a", "Island", "b"] -> “H I”
    public String quiz2() {
        return WORDS.stream().filter(list -> list.length() > 1)
                .map(list -> list.toUpperCase().substring(0,1))
